...
public class Level_8 extends AppCompatActivity {
    ...
    private View.OnClickListener getOclForCells(){
        return v->{
            if (Integer.parseInt(v.getTag().toString()) == count) {
                if (Integer.parseInt(v.getTag().toString()) != 25) {
                    count++;
    ...
    public void startResultsDialog() {
        ...
        if(isWin) new DBHelper(this).setOpenTaskNumberIs(9,t.time);

        View.OnClickListener OnClickListener = v -> {
            if (v.getId() == R.id.repeatResultsDialog) {
                if(isWin) {
                    startActivity(new Intent(getApplicationContext(), Level_8.class)); //REPEAT
                }else{
                ...
                Intent intent;// = null;
                if(isWin) {
                    intent = new Intent(getApplicationContext(),Level_9.class);
                }else{
                    intent = new Intent(getApplicationContext(), Level_8.class); //REPEAT
                }
