...
public class Level_7 extends AppCompatActivity {
    ...
    public void startResultsDialog() {
        ...
        if(isWin) new DBHelper(this).setOpenTaskNumberIs(8,t.time);


        View.OnClickListener OnClickListener = v -> {
            if (v.getId() == R.id.repeatResultsDialog) {
                if(isWin) {
                    startActivity(new Intent(getApplicationContext(), Level_7.class)); //REPEAT
                }else{
                ...
                Intent intent;// = null;
                if(isWin) {
                    intent = new Intent(getApplicationContext(),Level_8.class);
                }else{
                    intent = new Intent(getApplicationContext(), Level_7.class); //REPEAT
                }
    ...
    private void makeTask() {
        int[] array_of_numbers = Cells.getRandomArrayOfNumbers(16);
